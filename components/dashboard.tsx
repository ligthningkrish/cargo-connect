...
function BookingModal({ container, onClose, onBooked }: { container: Container; onClose: () => void; onBooked: (boxes: number) => void }) {
...
export function Dashboard({ role }: { role: Role }) {
  return <AppShell role={role}>{tab => role === "trader" ? <Trader tab={tab} /> : <Admin tab={tab} />}</AppShell>;
}
